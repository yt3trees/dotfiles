...
### Phase 0: モード確定（挿絵指示が出た場合のみ）

ユーザーから明示的に挿絵指示があった場合に限り、Phase 1 に入る前に `script/check_config.ps1` を実行して実生成モードか fallback モード（プレースホルダのみ）かを確定させる。style 選択・命名規則の合意もこのタイミングで行う。詳細は後述「挿絵」節。挿絵指示が無い場合は本 Phase をスキップして Phase 1 へ。
...
1. 大見出し（章）の設計: 採用する版の章数レンジに従う（簡易版 6〜8 / 標準 10〜12 / 本格版 12〜15）。レンジ上限を超えてはいけない。超える場合は2冊に分けるべき内容
...
```
## 第N章: [章タイトル] — [章副題（任意・章単位の切り口フレーズ。扉のサブタイトルとは別物）]
...
各章の目標字数は採用する版の「1章あたり」欄（下記分量表）に従う。節は 3〜5個。
...
挿絵を入れると決まったら、Phase 1 に入る前(= Phase 0)に まず `script/check_config.ps1` を実行して実生成モードか fallback モード(プレースホルダのみ)かを確定させる。モードが決まらないと style 選択や命名規則の合意も進められないため、ここで分岐する。
...
- [ ] 採用した版の章数レンジ上限を超えていない（簡易版 ≤8 / 標準 ≤12 / 本格版 ≤15）
- [ ] 各節 (`### N.X`) が **1,500字以上**
